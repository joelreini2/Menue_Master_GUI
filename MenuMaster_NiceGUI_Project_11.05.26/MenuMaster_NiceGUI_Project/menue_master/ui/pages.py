...
def seiten_erstellen(controller: MenuMasterController) -> None:
    ...
    with ui.tab_panels(tabs, value=tab_rezepte).classes("w-full p-4"):
        ...
        with ui.tab_panel(tab_wochenplan):
            wochenplanseite(controller)
        with ui.tab_panel(tab_einkaufsliste):
            einkaufslistenseite(controller)
        with ui.tab_panel(tab_historie):
            historienseite(controller)
...
def wochenplanseite(controller: MenuMasterController) -> None:
...
def einkaufslistenseite(controller: MenuMasterController) -> None:
...
def historienseite(controller: MenuMasterController) -> None:
